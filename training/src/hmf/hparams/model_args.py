...
class BaseModelArguments:
    ...
    preserve_tokenizer_config: bool = field(
        default=True,
        metadata={
            "help": (
                "Whether to preserve the original tokenizer_config.json when saving checkpoints."
            )
        },
    )
...
class ModelArguments(
    ...
    config_overrides_json: str | None = field(
        default=None,
        metadata={
            "help": (
                "Override some existing default config settings. Example: "
                "'{\"rope_parameters\": {\"rope_theta\": 5000000}, \"max_position_embeddings\": 131072}'"
            )
        },
    )
